...
export function AdminCategoriesPage({ navigation }: { navigation: ReactNode }) {
  ...
  return <PageLayout navigation={navigation}><Box sx={{ p: { xs: 3, sm: 6 } }}><CategoryManager categories={categories} onCreate={create} />{status && <Typography sx={{ mt: 2 }}>{status}</Typography>}</Box></PageLayout>
}
